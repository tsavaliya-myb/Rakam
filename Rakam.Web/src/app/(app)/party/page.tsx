...
export default function PartyPage() {
  ...
  function handleSearchChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    (handleSearchChange as any)._timer = setTimeout(() => setDebouncedSearch(val), 350);
  }
  ...
  function handleSubmit(data: PartyFormValues) {
    ...
    if (editParty) {
      ...
      createParty.mutate(dto, { onSuccess: () => setModalOpen(false) });
    }
  }
    ...
    <div className="p-6">
              ...
              {parties.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-16 text-center text-sm text-muted-foreground">
                    {debouncedSearch ? "No parties match your search." : "No parties yet. Add your first party."}
                  </td>
                </tr>
              ) : (
                parties.map((party) => {
                    ...
                    <tr key={party.id}
                      ...
                      <td className="px-4 py-3">
                        <div className="relative">
                          ...
                          >
                            {deleteParty.isPending && deleteParty.variables === party.id ? (
      ...
      {!isLoading && !isError && (
        <p className="text-xs text-muted-foreground mt-3 px-1">
          ...
          <span className="font-semibold text-foreground">{total}</span> parties
        </p>
      )}
