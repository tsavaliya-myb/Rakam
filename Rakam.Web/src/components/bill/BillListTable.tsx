...
import {
  CreditCard, Printer, Download, FileText,
  Copy, MoreHorizontal, ChevronUp, ChevronDown,
  ChevronsUpDown, Zap, Pencil, Trash2,
} from "lucide-react";
...
function ActionMenu({ bill, onRecordPayment, onEdit, onDelete, onDuplicate, onCreditNote, onCreateEway, onPrint, onDownload }: {
  bill: Bill;
} & Omit<BillListTableProps, "data">) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <button
        ...
        title={bill.status === "PAID" ? "Paid" : "Record Payment"}
      ...
      <div className="relative">
        ...
        {moreOpen && (
          <>
            ...
            <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-xl shadow-xl border border-border py-1 z-50 animate-fade-in">
              <button
                onClick={() => { onCreateEway(bill); setMoreOpen(false); }}
                className="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-medium text-foreground hover:bg-brand-50 hover:text-brand-700 transition-colors"
              >
                <Zap size={13} /> Create E-way Bill
              </button>
              <button
...
export function BillListTable({
  ...
  const columns = useMemo<ColumnDef<Bill>[]>(
    () => [
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <span className={cn(
              ...
              {v === "TAX_INVOICE" ? "Tax Invoice" : "Job Challan"}
            </span>
          );
        },
      },
      ...
      },
      {
        ...
        cell: ({ getValue, row }) => {
          ...
          if (status === "PAID") return <span className="text-xs text-muted-foreground">—</span>;
          return (
            <span className={cn(
    ...
    [onRecordPayment, onEdit, onDelete, onDuplicate, onCreditNote, onCreateEway, onPrint, onDownload]
  );
          ...
          <tbody>
            {table.getRowModel().rows.length === 0 ? (
              ...
              table.getRowModel().rows.map((row) => (
                <tr
                  key={row.id}
                  className="border-b border-border last:border-0 hover:bg-brand-50/40 transition-colors"
                >
